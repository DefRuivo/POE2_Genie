...
const RecipeCard: React.FC<Props> = ({ recipe, dishImage, setDishImage }) => {
  ...
  return (
    <article className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-200 overflow-hidden">
      <div className="relative min-h-[350px] bg-slate-900 overflow-hidden flex flex-col items-center justify-center">
        {dishImage ? (
          ...
          <div className="p-10 text-center space-y-6 z-10">
            <h3 className="text-4xl font-black text-white tracking-tighter leading-none mb-4">{recipe.recipe_title}</h3>
      ...
      <div className="p-8 md:p-14">
        <div className="bg-slate-50 border border-slate-200 p-6 rounded-3xl mb-12">
          ...
          <p className="text-slate-600 text-sm italic">"{recipe.analysis_log}"</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-12 gap-16">
          <div className="md:col-span-5 space-y-10">
            <h4 className="text-xl font-black text-slate-900 flex items-center gap-3">Da Despensa</h4>
          ...
          <div className="md:col-span-7">
            <h4 className="text-xl font-black text-slate-900 flex items-center gap-3 mb-10">Passo a Passo</h4>
            <div className="space-y-10 relative">
              <div className="absolute left-[15px] top-4 bottom-4 w-0.5 bg-slate-100"></div>
              {recipe.step_by_step.map((step, idx) => (
                <div key={idx} className="relative pl-12">
                  <div className="absolute left-0 w-8 h-8 bg-white border-4 border-indigo-500 rounded-full flex items-center justify-center font-black text-xs text-indigo-600 z-10">{idx + 1}</div>
