...
const HouseholdSection: React.FC<Props> = ({ household, setHousehold, activeDiners, setActiveDiners }) => {
  ...
  const handleRemoveMember = async () => {
    ...
    try {
      ...
      setHousehold(household.filter(m => m.id !== memberToDelete));
  ...
  return (
    <section className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
      ...
      <ConfirmDialog
        ...
        title="Remove Member?"
        message="Are you sure you want to remove this member from your household? This action cannot be undone."
      />
    </section>
  );
};
