                        ...
                        <div className="space-y-4 mb-6">
                            <div className="flex p-1 bg-slate-200/50 rounded-xl">
                                <button
                                    ...
                                    className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${statusTab === 'pending' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                                >
                                    {t('shopping.pendingTab') || fallbackLabels.pending} ({pendingCount})
                                </button>
                                <button
                                    onClick={() => setStatusTab('completed')}
                                    className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${statusTab === 'completed' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                                >
                                    {t('shopping.completedTab') || fallbackLabels.completed} ({completedCount})
                                </button>
                            </div>

                            <div className="text-xs font-semibold text-slate-500 bg-slate-100 px-3 py-2 rounded-lg space-y-1">
                                <p>
                                    <span className="font-black text-amber-700 mr-1">{t('shopping.statusPending') || fallbackLabels.pending}:</span>
                                    {t('shopping.pendingHelp') || 'Items still needed for your builds.'}
                                </p>
                                <p>
                                    <span className="font-black text-emerald-700 mr-1">{t('shopping.statusCompleted') || fallbackLabels.completed}:</span>
                            ...
                            <div className="flex p-1 bg-slate-200/50 rounded-xl">
                                <button
                                    ...
                                    className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${sourceFilter === 'all' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                                >
                                    {t('shopping.filterAll') || 'All'}
                                </button>
                                <button
                                    onClick={() => setSourceFilter('manual')}
                                    className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${sourceFilter === 'manual' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                                >
                                    {t('shopping.filterMyList') || 'My Checklist'}
                                </button>
                                <button
                                    onClick={() => setSourceFilter('recipe')}
                                    className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${sourceFilter === 'recipe' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                                >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {filteredItems.length === 0 && (
                                <div className="col-span-full text-center py-20 opacity-50">
                                    <i className="fas fa-gem text-4xl mb-4 text-slate-300"></i>
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <div className="flex items-baseline gap-2">
                                                    ...
                                                    <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full border ${item.checked ? 'text-emerald-700 bg-emerald-50 border-emerald-100' : 'text-amber-700 bg-amber-50 border-amber-100'}`}>
                                                        {item.checked
